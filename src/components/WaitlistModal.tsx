...
const WaitlistModal = ({ trigger, source = "unknown" }: WaitlistModalProps) => {
  ...
  const trackedTrigger = useMemo(() => {
    const handleTriggerClick = () => {
      trackEvent("join_waitlist_click", {
        source,
        language,
      });
      trackClick("waitlist_trigger", source, { language });
    };

    if (trigger && isValidElement(trigger)) {
      const triggerElement = trigger as ReactElement<{ onClick?: () => void }>;
    ...
    return (
      <button
        ...
        className="inline-flex h-14 items-center rounded-full bg-primary px-7 font-mono-ui text-xs font-semibold uppercase tracking-[0.24em] text-primary-foreground transition-transform hover:-translate-y-0.5 whitespace-nowrap"
      >
        {t.cta.button.replace(/_/g, " ")}
      </button>
    );
  }, [language, source, t.cta.button, trigger]);
  ...
  return (
    <Dialog
      ...
      onOpenChange={(nextOpen) => {
        setOpen(nextOpen);
        trackEvent(nextOpen ? "waitlist_modal_open" : "waitlist_modal_close", {
          source,
          language,
        });
      }}
      ...
      <DialogContent className="max-h-[85vh] max-w-[680px] overflow-y-auto border border-border bg-card p-0 text-foreground">
        ...
        <div className="p-4 md:p-6">
          <iframe
            ...
            title="Waitlist Form"
            onLoad={() => trackEvent("waitlist_form_view", { source, language })}
          />
        </div>
      </DialogContent>
    </Dialog>
